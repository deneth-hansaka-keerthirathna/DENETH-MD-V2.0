...
const clientstart = async() => {
    ...
    if (config().status.terminal && !sock.authState.creds.registered) {
        const phoneNumber = await question('Enter Your WhatsApp Number, Starting with 94:\nWhatsApp Number: ');
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'open') {
            ...
            let up = 
    `*DENETH-MD V2 IS ONLINE NOW* ✅\n\n` +
    `> 𝔘𝔰𝔢𝔯𝔫𝔞𝔪𝔢: ${sock.user?.name || 'Unknown'}\n` +
    `> 𝔓𝔯𝔢𝔣𝔦𝔵: [ . ]\n` +
    `> 𝔐𝔬𝔡𝔢: ${config().settings.mode || 'Public'}\n` +
    `> 𝔙𝔢𝔯𝔰𝔦𝔬𝔫: 2.0.0\n` +
    `> 𝔒𝔴𝔫𝔢𝔯: DenethDev®\n\n` +
    `_Thanks for using This Bot_ 💕\n` +
    `> 𝔅𝔬𝔱 𝔇𝔢𝔳𝔢𝔩𝔬𝔭𝔢𝔡 𝔟𝔶 𝔇𝔢𝔫𝔢𝔱𝔥𝔇𝔢𝔳®`;

sock.sendMessage(botNumber, {
    image: { url: `https://raw.githubusercontent.com/deneth-hansaka-keerthirathna/DENETH-Media/refs/heads/main/main2.jpg` },
    caption: up,
    contextInfo: {
        forwardingScore: 11,
        isForwarded: true,
